...
def api_ranking_refresh():
    ...
    miki = "xqGIPTs2EuF-EQ_G9-acJN2gaf3836pGqzGOurYNCc47MSM"
    kacper = "sDa8Mpkt5IkU4zqmqkIWiG3aeV3p-ybrpDEg9Z4hE6eGMJQ"
    czarek = "N1a6oE7owpEIh1Odmn1wujcwoEIhwwytk6vO2grAC_brLF4"
    michal = "DzwijJUpJmYKXi6_JQMW3NgHP1DyxN6DpeBwIr-ob5H83AA"
    ...
    print(miki_info, kacper_info, czarek_info, michal_info)
...
@app.route('/api/v2/gamemmr', methods=['GET'])
def api_mmr_v2():
    if 'apikey' in request.args:
        try:
            apikey = name = str(request.args['apikey'])
    ...
    def main():
        ...
        for i in range (0, 10):
            ...
            if(results['game'][i]['available']==1):
                results['game'][i]['mmr'] = game_info[i][2]
                if game_info[i][3] == 100:
                    results['game'][i]['team'] = "blue"
                else:
                    results['game'][i]['team'] = "red"
...
@app.route('/api/v2/ranking/refresh', methods=['GET'])
def api_ranking_refresh_v2():
    from riotwatcher import LolWatcher, ApiError
    import requests
    import time
    ...
    list_4 = [miki_info, kacper_info, czarek_info, michal_info]

    results = { 'ranking': []}

    for player in list_4:
        for i in range(len(player)):
            #print(player)
            temp = {
                player[i]['queueType']: 
                {
                    'nickname': player[i]['summonerName'],
                    'tier': player[i]['tier'],
                    'rank': player[i]['rank'],
                    'leaguePoints': player[i]['leaguePoints'],
                },
            }
            results['ranking'].append(temp)
